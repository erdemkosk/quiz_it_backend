...
const updateStatistic = async (req, res, next) => {
  try {
    ...
    const {success} = await memberService.updateStatistic({
      id,
      isRightAnswer,
      questionLevel,
    });
    ...
    return res.status(200).send(responseHelper.generateSuccessResponse({ message: 'Member statistic updated.', data: success }));
  }
  catch (error) {
    next(error);
  }
